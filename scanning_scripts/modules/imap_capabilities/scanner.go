...
type ScanResults struct {
	...
	// PreTLS is the targets response to CAPABILITY in plain (pre-tls) state
	PreTLS string `json:"preTLS,omitempty"`

	// PreTLSID is the targets response to ID in plain (pre-tls) state
	PreTLSID string `json:"preTLSID,omitempty"`

	// PostTLS is the targets response to CAPABILITY in encrypted (post-tls) state
	PostTLS string `json:"postTLS,omitempty"`

	// PostTLSID is the targets response to ID in encrypted (post-tls) state
	PostTLSID string `json:"postTLSID,omitempty"`
...
func RegisterModule() {
	var module Module
	_, err := zgrab2.AddCommand("imap_capabilities", "Fetches IMAP capabilities before and after STARTTLS", module.Description(), 143, &module)
	if err != nil {
		log.Fatal(err)
	}
}
...
func (scanner *Scanner) Scan(target zgrab2.ScanTarget) (zgrab2.ScanStatus, interface{}, error) {
	...
	// Step 1.1
	// Send ID-Command
	command = "1 ID (\"name\" \"zgrabscanner\" \"version\" \"1.0\" \"vendor\" \"fhms\")"
	result.Trace = append(result.Trace, "C: "+command)
	conn.Conn.SetReadDeadline(time.Now().Add(singleReadTimeout))
	ret, err = conn.SendCommand(command)
	...
	// Quit if target didnt "B OK" the STARTTLS
	if !strings.Contains(strings.ToUpper(ret), "B OK") {
	...
	// Step 4.1
	// Send ID-Command
	command = "2 ID (\"name\" \"zgrabscanner\" \"version\" \"1.0\" \"vendor\" \"fhms\")"
	result.Trace = append(result.Trace, "C: "+command)
	conn.Conn.SetReadDeadline(time.Now().Add(singleReadTimeout))
	ret, err = conn.SendCommand(command)
	if strings.Contains(strings.ToUpper(ret), "2 OK") {
		result.PostTLSID = ret
	}
	result.Trace = append(result.Trace, "S: "+strings.Trim(ret, "\r\n"))
	result.Status++
